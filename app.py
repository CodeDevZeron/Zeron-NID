...
IMGBB_API_KEY = os.getenv("IMGBB_API_KEY", "b402067f6bfc278112333ae2d974c093")
...
def generate_nid():
    ...
    sign = request.args.get("sign", "Signature")
    issue = request.args.get("issue", "০১/০১/২০২৫")
    ...
    # Load NID template (user must provide nid.png)
    img = Image.open("nid.png").convert("RGB")
    draw = ImageDraw.Draw(img)

    # Fonts (from fonts/ folder)
    font_bn = ImageFont.truetype("fonts/NotoSansBengali-Regular.ttf", 28)  
    font_en = ImageFont.truetype("fonts/DejaVuSans.ttf", 24)
